...
func loopback() error {
	...
	if err != nil { return err }
	if err = unix.IoctlIfreq(fd, unix.SIOCGIFFLAGS, ifr); err != nil { return err }
	ifr.SetUint16(ifr.Uint16() | unix.IFF_UP)
	return unix.IoctlIfreq(fd, unix.SIOCSIFFLAGS, ifr)
}

func restrictions(p policy) error {
	if p.Port != 0 && (p.Port < 1024 || p.Port > 65535) { return errors.New("invalid loopback port") }
	if p.Port != 0 {
		if err := loopback(); err != nil { return fmt.Errorf("loopback: %w", err) }
	}
	var rules []landlock.Rule
	for _, name := range p.Read {
		info, err := os.Stat(name)
		if err != nil { return err }
		if info.IsDir() { rules = append(rules, landlock.RODirs(name)) } else { rules = append(rules, landlock.ROFiles(name)) }
	}
	for _, name := range p.Write {
		info, err := os.Stat(name)
		if err != nil { return err }
		if info.IsDir() { rules = append(rules, landlock.RWDirs(name).WithRefer()) } else { rules = append(rules, landlock.RWFiles(name)) }
	}
	if p.Port != 0 { rules = append(rules, landlock.BindTCP(uint16(p.Port)), landlock.ConnectTCP(uint16(p.Port))) }
	...
	if err := landlock.V4.Restrict(rules...); err != nil { return fmt.Errorf("landlock: %w", err) }
	for cap := 0; cap <= 40; cap++ {
		if err := unix.Prctl(unix.PR_CAPBSET_DROP, uintptr(cap), 0, 0, 0); err != nil && err != unix.EINVAL { return err }
	}
...
func main() {
	if len(os.Args) < 4 { fail(errors.New("expected policy and command")) }
	inner := os.Args[1] == "--inner"
	offset := 1
	if inner { offset++ }
	if len(os.Args) <= offset+1 { fail(errors.New("missing command")) }
	var p policy
	if err := json.Unmarshal([]byte(os.Args[offset]), &p); err != nil { fail(err) }
	if inner {
		if os.Getpid() != 1 { fail(errors.New("missing PID namespace")) }
		if err := restrictions(p); err != nil { fail(err) }
	} else {
		if os.Getuid() == 0 { fail(errors.New("outer worker must not run as root")) }
...
func run(cmd *exec.Cmd) {
	if err := cmd.Run(); err != nil {
		...
		if errors.As(err, &exited) && exited.ExitCode() >= 0 { os.Exit(exited.ExitCode()) }
		fail(err)
	}
}
