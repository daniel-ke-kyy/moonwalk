...
// Native live preview polls on a fixed interval. Share pending GETs when a
// low-CPU cloud response takes longer than that interval, without caching data.
export const previewPollingScript = `(function () {
  const originalFetch = window.fetch.bind(window);
  let pending;
  window.fetch = function (input, options) {
    if (typeof input !== 'string' || !input.endsWith('/api/slides') || (options && options.method && options.method !== 'GET')) {
      return originalFetch(input, options);
    }
    if (!pending) pending = originalFetch(input, options).finally(function () { pending = null; });
...
export function mountPreviewProxy(router, store, controller, bearerToken) {
  ...
  router.use('/projects/:id/preview', async (req, res) => {
    ...
    if (req.path === '/static/moonwalk-readonly.js') return res.type('application/javascript').send(previewPollingScript + (editable ? annotationScript : readOnlyScript))
